...
async function init() {
    ...
    updateUI();
    loadLinks();
    checkPromoCost(); // Initialize cost indicator
}
...
function openAdmin() {
    ...
    if (password === "Propetas12") {
        ...
        loadAdminData();
        loadAdminLinks(); // Load links for management
    } else if (password !== null) { // User clicked cancel, not closed
...
function loadAdminData() {
    ...
    db.ref('deposits/pending').on('value', snap => {
        ...
        if (!snap.exists()) {
            cont.innerHTML += "<p class='text-center text-gray-600 text-[9px] py-4'>No pending deposits.</p>";
            return;
        }
        snap.forEach(c => {
            const d = c.val();
    ...
    db.ref('withdrawals/pending').on('value', snap => {
        ...
        cont.innerHTML = "<h4 class='text-[10px] text-red-500 font-black mb-3 mt-6 uppercase'>Pending Withdrawals</h4>";
         if (!snap.exists()) {
            cont.innerHTML += "<p class='text-center text-gray-600 text-[9px] py-4'>No pending withdrawals.</p>";
            return;
        }
        snap.forEach(c => {
            const w = c.val();
...
function loadAdminLinks() {
    ...
    const cont = document.getElementById('admin-links-list');
    db.ref(`links/${category}`).on('value', snap => {
        cont.innerHTML = "";
        if (!snap.exists()) {
            cont.innerHTML += "<p class='text-center text-gray-600 text-[9px] py-4'>No links in this category.</p>";
            return;
        }
        snap.forEach(linkSnap => {
            const link = linkSnap.val();
            cont.innerHTML += `<div class='bg-black p-3 rounded-xl mb-2 flex justify-between items-center text-[9px] border border-white/5'>
                <div class='truncate max-w-[70%]'><p class='font-bold'>${link.url}</p><p class='opacity-50 truncate'>${link.desc.substring(0, 50)}...</p></div>
                <button onclick="deleteAdminLink('${category}','${linkSnap.key}')" class='bg-red-600 px-2 py-1 rounded font-black'>DELETE</button>
            </div>`;
        });
    });
}
...
async function postAdminGlobal() {
    ...
    const category = 'watch'; // Default category for admin global posts
    if (!url || !desc) return alert("URL and Description are required.");

    await db.ref(`links/${category}`).push({ 
        url, desc, clicks: 10000, reward: 0.03, time: 20, owner: 'admin' 
    });
    alert("10,000 Click Task Injected!");
    document.getElementById('adminUrl').value = "";
    document.getElementById('adminDesc').value = "";
    loadAdminLinks(); // Refresh list if needed
}

async function deleteAdminLink(category, linkId) {
    if (!confirm("Are you absolutely sure you want to delete this task? This action cannot be undone.")) return;
    await db.ref(`links/${category}/${linkId}`).remove();
    alert("Task deleted.");
    loadAdminLinks(); // Refresh list
}
...
async function processLinkSubmission() {
    ...
    if (!config) return alert("Invalid task type selected.");
    ...
    document.getElementById('subUrl').value = ""; // Clear form
...
async function submitDeposit() {
    ...
    if (!amt || amt <= 0 || !ref) return alert("Please fill in a valid amount and transaction reference.");
    ...
    document.getElementById('depAmount').value = ""; // Clear form
    document.getElementById('depRef').value = "";
}

async function requestWithdraw() {
    const amt = parseFloat(document.getElementById('witAmount').value);
    const acc = document.getElementById('witAccount').value.trim();
    ...
    document.getElementById('witAmount').value = ""; // Clear form
    document.getElementById('witAccount').value = "";
    updateUI();
}

// --- REFERRAL SYSTEM ---
async function submitReferral() {
    const code = document.getElementById('inputRefCode').value.trim().toUpperCase();
    if (code === userData.myRefCode) return alert("You cannot use your own code.");
    if (code.length !== 6) return alert("Referral codes are exactly 6 letters.");
    
    // Fetch users based on the entered code
    const usersRef = db.ref('users');
    const snapshot = await usersRef.orderByChild('myRefCode').equalTo(code).once('value');

    if (snapshot.exists()) {
        const refId = Object.keys(snapshot.val())[0];
        // Prevent double referral assignment or self-referral attempt
        if (userData.referredBy && userData.referredBy === refId) {
            alert("This referral is already active.");
            return;
        }
        
        // Update user's referredBy field and increment referrer's referral count
        await usersRef.child(userId).update({ referredBy: refId });
        await usersRef.child(refId).child('referrals').transaction(c => (c || 0) + 1);
        
        alert("Referral code applied!");
        location.reload(); // Reload to reflect changes (e.g., hiding the input box)
    } else { 
        alert("Invalid referral code."); 
    }
}

// --- LINK MANAGEMENT (User & Admin) ---
...
// --- EVENT LISTENERS ---
document.getElementById('adminLinkCategory')?.addEventListener('change', loadAdminLinks);
